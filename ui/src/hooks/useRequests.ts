...
export function useRequests() {
  ...
  // Re-fetch whenever the active project changes so switching projects always
  // loads the correct history — even when filter values stay the same.
  const projectPath = useProxyStore((s) => s.project?.path)
  ...
  }, [host, method, setRequests, projectPath])
}
